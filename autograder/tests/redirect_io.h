...
void execute_test(const string& file_name, VoidFunction func) {
    ...
    while(it_1 != end(output_test)) {
        if (it_1->starts_with("[REGEX]")) {
            std::stringstream ss_expected(*it_1);
            std::stringstream ss_obtained(*it_2);
            std::string expected;
            std::string obtained;
            while (std::getline(ss_expected, expected) && std::getline(ss_obtained, obtained)) {
                std::smatch m;
                std::regex re{expected.substr(std::strlen("[REGEX]"))};
                REQUIRE(std::regex_match(obtained, m, re));
            }
            ++it_1; ++it_2;
        }
        else {
